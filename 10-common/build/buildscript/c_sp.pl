...
for ( my $i = 0 ; $i < @ARGV; $i++ )
{
	if ( $ARGV[$i] =~ /^-/ ) # "-"标识options
	{
		if ( $spoptions eq "" )
		{
			...
			my @preoptions = qw(u t b c f r n);  ####增加t 2017-3-21add
	...
	elsif (( $ChangefileP eq "" ) && ( $ARGV[$i] =~ /^http+/ )) # 第四个参数为指定发布路径 2017-3-21add
	{
		$ChangefileP = $ARGV[$i];
		print "!!!!\n";
		print $ChangefileP;
...
if ( $spoptions =~ /r/ )
{
	...
	if ( $spspecRP eq "" ) # 当用户没有指定发布路径 , 根据发布规则在发布路径后加日前文件夹/日前时间文件夹以区分每次发布
	{
		...
		if ( $spoptions =~ /t/ ) 
		{
			...
			$p = &revisepath(0,$builddate."-TMP/".$UCMprj."_".$version."_R".$version_name."#R".$tmpfile_R);
		    #$p = &revisepath(0,$builddate."-tmp_".$tmpRPn."/".$UCMprj."_".$version."_R".$version_name."#T".$buildtime);
		}
		###-----------------------------------------------
	    $RP = $ReleaseP.$p;	
		...
		# if ( $modulelist =~ /,/ )
		# {
			# my @modules_r = split /\,/,$modulelist;
			# my $p = &revisepath(0,"$builddate-SP/$builddatetime-$modules_r[0]"."+++");
			# $RP = $ReleaseP.$p;
		# }
		# else
	  # {
			# my $p = &revisepath(0,"$builddate-SP/$builddatetime-$modulelist");
			# $RP = $ReleaseP.$p;
	  # }
	}
	else
