...
sub getenv
{
	...
	foreach $envkey (@envkeys) # 逐个处理传入的环境关键字
	{
		...
		elsif (( $envkey =~ /^JAVA_HOME[\d\._]*$/i ) ||( $envkey =~ /^PPC[\d\._]*$/i ) || ( $envkey =~ /^ARM[\d\._]*$/i ) || ( $envkey =~ /^EQT[\d\._]*$/i ) || ( $envkey =~ /^DAVINCI[\d\._]*$/i ))
		{
			foreach $f (@found) # 逐行处理环境关键字的行
			{
				my @value = &getvalue("=",$f); # 分离环境变量value[0]与环境变量值value[1]
...
sub afterprocess
{
	...
	if ( $options =~ /b/ )
	{
		...
		if ( open(BUILDINFO,">$verworkP"."BuildInfo_".$OSstring."_".$builddatetime.".txt") )
		{
			...
			if ($spoptions =~ /t/) 
			{
			    print BUILDINFO "Tmp Change File URL : $ChangefileP\n";
			}
			###------
...
###拷贝解压缩临时文件2017-3-22add
sub copy_tmpfile
{
    my ($changefile_url) = @_;
	chdir $SnapviewP; #切换到工作副本目录
	
	&command('mkdir','tmp'); #创建tmp文件夹
	#system("svn checkout $changefile_url    .   --username svnsuperadmin --password 'Svn123admin' --no-auth-cache "); #下载附件包
	system("svn export $changefile_url    .   --username hanjian --password hanjian123163 --no-auth-cache "); #下载附件包
	my @tmpfile = split(/\//,$changefile_url);
	my $zipfile = @tmpfile[@tmpfile-1];
	&command('unzip',$SnapviewP.$zipfile,$SnapviewP.'tmp'); #解压缩
	print "\n!!!NOW unziping tmp file\n";
	#遍历
    use strict;
    use File::Spec;
...
#替换临时文件 ####2017-3-21add  ##2018-2-27添加删除文件规则
sub tmp_changefile
{
    my (@tmpfiles) = @_;
	chdir $SnapviewP; #切换到工作副本目录
	foreach $tmpfile(@tmpfiles)
	{
		...
		{
		   #修正tmp路径结尾的连接符斜杠
		   my $path_tmp=&revisepath(0,"tmp/"); 
		   #如果文件名结尾是#Del，删除tmp下这个文件
		   &command("del","f",$path_tmp.$tmpfile); 
           $tmpfile =~ s/#Del$//i; 		   
		}
		
		#重命名
		print "now rename $tmpfile,$tmpfile.'---'\n";
        rename $tmpfile,$tmpfile.'---';	
...
sub recory_tmpfile
{
	...
	#删除 还原文件
	print "\n%%%%%now recory tmp file\n";
	my (@tmpfiles,$tmpfilename) = @_;
	chdir $SnapviewP; #切换到工作副本目录
    &command("rd","tmp");
    &command("rd",$tmpfilename);	
	foreach $tmpfile(@tmpfiles)
	{
	    if ( $tmpfile =~ /#Del$/i )
		{
		...
		#删除
		unlink $SnapviewP.$tmpfile;
		print "\n%%%%%now deleting tmp file:$SnapviewP.$tmpfile\n";
		#重命名
		print "now rename $tmpfile.'---',$tmpfile\n";
        rename $tmpfile.'---',$tmpfile;		    
	}
...
#			print "WcheckF : $WcheckF\n";
		...
		&writelog($workpath."nopassfile.log","--- $module / $owner / $ownerM ---\n".$nopassinfo."\n") if ( !$checkfileS ); # 将校验未通过文件写入临时文件nopassfile.log
...
sub command
{
	# input: commandkey (update , exe , copy , del , updatecopy , pause,unzip,mv), command items
	# output: $vererrL
	...
	elsif ( $cmd eq "unzip" )  ###2017-3-22 add解压缩命令
	{
		print "Command : $cmd : $item0\n";
		if ( $OS )
		{
		    if ( $item0 =~ /zip$/)
			{
			    system("unzip   -o    $item0  -d  $item1  2>$vererrL");
			}
			if ( $item0 =~ /gz$/ )
			{
			    system("tar   zxvf    $item0  -C  $item1  2>$vererrL");
			}
			
		}
		else
		{
			system("winrar x $item0 $item1 -y 2>$vererrL");
		}
	}
	
	elsif ( $cmd eq "rd" )  ###2017-3-22 add
	{
		print "Command : $cmd : $item0\n";
		if ( $OS )
		{
			system("rm -rf    $item0    2>$vererrL");
		}
		else
		{
			system("rd /s /q $item0  2>$vererrL");
		}
	}
...
sub get_linux_version
{
  ...
  my @svnRev_l=`svn info . --username root --password kdckdc --no-auth-cache`;
...
#获取临时替换文件的版本号
sub get_tmpfile_version
{
    print '\n!!!! this is get_tmpfile_version\n';
	my ($chfile_url) = @_;
	print '$chfile_url\n';
    my $tmpfile_version;
	my @tmp_version=`svn info $chfile_url --username hanjian --password hanjian123163 --no-auth-cache`;
	my @tmp_v=split ":",$tmp_version[8];
	$tmpfile_version=$tmp_v[1];
	$tmpfile_version =~ s/\s//g;
	return $tmpfile_version;	
}
