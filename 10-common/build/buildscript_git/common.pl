...
###			}
			if ( open(BASELINE,$verworkP."baseline.log") )
			{
				@blfile = <BASELINE>;
				close(BASELINE); 
				foreach (@blfile){
				...
				print "blfile as array:" . "@blfile" . ">>>>>>>>>>\n" ;
				print BUILDINFO ("\n============================= Stream BaseLine Info =============================\n"." @blfile"."\n");
				undef(@blfile);
			}
			# 编译时间写入编译信息
...
sub command
{
	...
	elsif ( $cmd eq "update" )
	{
		...
		######system("cleartool update -f -ove -log $item1 $item0 2>$vererrL");
            # Added by suling temporarily to satigy git application
            my @sources = getvalue(" ", $item0);
            print "cmo debugging>>>\n   SnapviewP:$SnapviewP>>>\n";
            foreach $source ( @sources ){
            	&cleanerror;
                ### if( $source =~ /(\Q$SnapviewP\EProfessional_VOB\Edriver-git\Ekernel)|(\Q$SnapviewP\EProfessional_VOB\Edriver-git\Enip)|(\Q$SnapviewP\EProfessional_VOB\Edriver-git\Euboot)|(\Q$SnapviewP\EProfessional_VOB\Edriver-git\Eplatform)/ ) {
                if( $source =~ /(Professional_VOB\/driver-git\/kernel)|(Professional_VOB\/driver-git\/nip)|(Professional_VOB\/driver-git\/uboot)|(Professional_VOB\/driver-git\/platform)/ ) {   
                 print "source at git:$source>>>>>>>>>\n";
                    my $branch_name = "20131028_TPS";
                    system( "cd $source && git reset --hard && git clean -df && git pull origin  $branch_name 2>$vererrL" );
         				
   	    my $commit = `cd $source && git show -s HEAD 2>$vererrL`;
		    if ( !&geterror )
		    {
		    	$commit =~ s/commit (\w{40}).*/$1/s;   #/s修饰词令.匹配换行符并且忽略废弃了的$*
		    	$commit = $source . " ==> " . $commit;
		    	$commit =~ s/^ (.*)/$1/; 
		    	print "commit:$commit>>>>>>>\n";
		    	&writelog($verworkP."baseline.log",$commit."\n");
...
#   print "source at clearcase:$source\n";
#   system("cleartool update -f -ove -log $item1 $source 2>$vererrL");
                }    
            }
  }  
