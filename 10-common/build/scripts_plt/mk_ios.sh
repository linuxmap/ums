...
input_path=$topdir/driver-svn/kernel
...
# for not to update autoconf.h when change .config 2017.10.19 MHQ   
    cd $kernel_build_top 
    make ARCH=$arch_style distclean
...
{		
	  ...
	  order=b			
		...
		source_nfs=$topdir/driver-svn/source/standard/$full_cpu_name/nfs
   ...
   ##prepare linux.jffs2
    ...
    cp -rfd $topdir/driver-svn/nip/source/snmp/mibs $target_nfs/etc/ ||exit 1
    ...
    if [ $cpu_platform = "davinci" ] ; then
		...
		$topdir/driver-svn/source/tools/mkfs.jffs2 -e 0x2000 -${order} -p --disable-compressor=rtime --compressor-priority=60:zlib -r $target_nfs -o $version_path/linux.jffs2 || exit 1
	  ...
	  if [ "$cpu_platform" = "8313" ] || [ "$cpu_platform" = "8377" ] || [ "$1" = "is3-mpc8548" ];  then
		...
		#create update.linux
	  $topdir/driver-svn/source/tools/updata linux.ios linux.jffs2 update.linux
