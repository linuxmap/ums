 ...
 * Default Value: 90—3 messages per transaction
 * Remark: The parameter value must be greater than zero
 *
 * messagePoolPageSize
 * -------------------
 ...
 * Default Value: 100—2 per call, 2 per transaction, 20 for general usage
 * Remark: The parameter value must be greater than zero
 *
 * generalPoolPageSize
 * -------------------
 * The size of a page in the general pool.
 * Default Value: 1024—half the size of a message pool page
 * Remark: The parameter value must be greater than zero
 *
 ...
 * Remark: This parameter can be set to zero or to “0.0.0.0” to indicate a default
 ...
 * Default Value: 31—full logging information
 *
 * coreLogFilters
 * ---------------
...
typedef struct {
	...
	RvSipEncryptEv	pfnEncryptEvHandler;	//encrypt or decrypt sip packets[quantum]
    // For SIP port range support
	RvUint32			portNumberStart;
	RvUint32			portNumberEnd;
}RvSipStackCfg;
