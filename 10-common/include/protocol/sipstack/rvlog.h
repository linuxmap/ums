...
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_EXCEP)) RvLogTextExcep funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextExcep(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogError(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_ERROR)) RvLogTextError funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextError(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogWarning(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_WARNING)) RvLogTextWarning funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextWarning(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogInfo(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_INFO)) RvLogTextInfo funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextInfo(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogDebug(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_DEBUG)) RvLogTextDebug funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextDebug(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogEnter(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_ENTER)) RvLogTextEnter funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextEnter(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogLeave(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_LEAVE)) RvLogTextLeave funcParams;}
RVCOREAPI RvStatus RVCALLCONV RvLogTextLeave(
    IN  RvLogSource*        source,
    IN  const char*         line, ...);

#define RvLogSync(source, funcParams) \
{if ((source != NULL) && RvLogIsSelected(source, RV_LOGLEVEL_SYNC)) RvLogTextSync funcParams;}
