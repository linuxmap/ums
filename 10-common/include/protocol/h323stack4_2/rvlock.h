...
#include "rvlog.h"
    ...
    (RV_LOCK_TYPE != RV_LOCK_WIN32_CRITICAL) && (RV_LOCK_TYPE != RV_LOCK_SYMBIAN) && \
    (RV_LOCK_TYPE != RV_LOCK_MANUAL) && (RV_LOCK_TYPE != RV_LOCK_MOPI) && \
 ...
 * RETURN  : RV_OK if successful otherwise an error code. 
 */
RvStatus Rv2LockSourceConstruct(
	IN RvLogMgr	*logMgr);
 ...
 */
RVCOREAPI
RvStatus RVCALLCONV Rv2LockConstruct(
	IN  RvLogMgr	*logMgr, 
	OUT	RvLock		*lock);
 ...
 */
RVCOREAPI
RvStatus RVCALLCONV Rv2LockDestruct( 
	IN  RvLock *lock,
	IN  RvLogMgr* logMgr);
 ...
 */
RVCOREAPI
RvStatus RVCALLCONV Rv2LockGet(
	IN RvLock	*lock,
	IN RvLogMgr	*logMgr);
 ...
 */
RVCOREAPI
RvStatus RVCALLCONV Rv2LockRelease(
	IN RvLock	*lock, 
	IN RvLogMgr	*logMgr);
 ...
 */
RvStatus Rv2LockSetAttr(
	IN RvLockAttr *attr,
	IN RvLogMgr   *logMgr);

#else
/* If none is set then none of these functions do anything */
#define Rv2LockConstruct(_lg,_l) (*(_l) = RV_OK)
#define Rv2LockDestruct(_l,_lg) (*(_l) = RV_OK)
#define Rv2LockGet(_l,_lg) (*(_l) = RV_OK)
#define Rv2LockRelease(_l,_lg) (*(_l) = RV_OK)
#define Rv2LockSetAttr(_l,_lg) (*(_l) = RV_OK)
#endif
