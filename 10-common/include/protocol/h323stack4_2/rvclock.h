...
#include "rvlog.h"
    ...
    (RV_CLOCK_TYPE != RV_CLOCK_SYMBIAN)  && (RV_CLOCK_TYPE != RV_CLOCK_MOPI))
 ...
 * RETURN  : RV_OK if successful otherwise an error code.
 */
RvStatus Rv2ClockSourceConstruct(
	IN RvLogMgr* logMgr);
 ...
 */
RVCOREAPI 
RvInt32 RVCALLCONV Rv2ClockGet(
    IN  RvLogMgr  *logMgr,
    OUT RvTime    *t);
 ...
 */
RVCOREAPI 
RvInt32 RVCALLCONV Rv2ClockResolution(
    IN  RvLogMgr *logMgr,
    OUT RvTime   *t);
 ...
 */
RVCOREAPI
RvStatus RVCALLCONV Rv2ClockSet(
    IN const RvTime *t,
	IN RvLogMgr     *logMgr);
