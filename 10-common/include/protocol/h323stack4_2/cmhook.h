...
typedef HOOK_PROCESS
	(RVCALLCONV*cmHookDoSendT)(IN HPROTCONN hConn,IN int nodeId,IN RvBool error);
typedef HOOK_PROCESS
	(RVCALLCONV*cmHookDoRecvT)(IN HPROTCONN hConn,IN int nodeId,IN RvBool error);
typedef HOOK_PROCESS
	(RVCALLCONV*cmHookDoSendToT)(IN HPROTCONN hConn,IN int nodeId,IN int addrTo,IN RvBool error);
typedef HOOK_PROCESS
	(RVCALLCONV*cmHookDoRecvFromT)(IN HPROTCONN hConn,IN int nodeId,IN int addrFrom,IN RvBool multicast,IN RvBool error);
...
#if defined _ENABLE_QUANTUM_PROJECT_ && defined _KDV_RV_H323_STACK_
   cmHookDoSendT        hookDoSend;
   cmHookDoRecvT        hookDoRecv;
   cmHookDoSendToT      hookDoSendTo;
   cmHookDoRecvFromT    hookDoRecvFrom;
#endif
