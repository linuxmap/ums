...
typedef void (*cmAutoRASACF)(void* pcallElem, RvUint32 dwPeerQuantumID, RvUint32 dwPeerIP);
#endif
...
typedef struct __cmElem
{
	...
	/* KEDA H.323 */
	RvBool		isKeda323Used;	/* Are we using KEDA H.323 for Sending Operation? */
	RvBool		isAutoKeda323;	/* Do we have auto KEDA H.323 enabled for Receive Operation ? */

	int         nTosValue;  //To set socket option of IP_TOS when HostListening

#ifdef _ENABLE_QUANTUM_PROJECT_
	RvUint32            m_dwMyQuantumID;
	cmAutoRASACF        cmACFCallback;
#endif
	RvBool	            IsQuantumEncrypt;
