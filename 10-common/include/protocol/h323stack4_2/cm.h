...
#ifdef _KDV_RV_H323_STACK_
	cmCallStateModeDisconnectedCustom,
	cmCallStateModeDisconnectedAdaptiveBusy,

#ifdef _ENABLE_QUANTUM_PROJECT_
	cmCallStateModeQTDevConnectionBroken,
	cmCallStateModeGetSignalEncryptKeyFailed,
#endif
...
typedef enum
{
	...
	cmReasonTypeQTDevConnectionBroken,
	cmReasonTypeGetSignalEncryptKeyFailed
#endif
...
#if defined _ENABLE_QUANTUM_PROJECT_ && defined _KDV_RV_H323_STACK_
RVAPI int RVCALLCONV cmCallDropQT(IN HCALL hsCall, cmReasonType emReasonType);
#endif
...
RVAPI void RVCALLCONV kdv323stackhelp();
RVAPI void RVCALLCONV radver();
RVAPI void RVCALLCONV h323sver();
 ...
 * RvSetKeda323
 * ----------------------------------------------------------------------------
 * General		: Change the stack library protocol mode (to KEDA H.323). Add by Jacky Wei
 *
 * Return Value	: RV_OK  - if successful.
 *               Other on failure
 * ----------------------------------------------------------------------------
 * Arguments	:
 * Input		: hApp         - HAPP handle of the stack which was retrieved from cmInitialize.
 *				  bKDV323      - Indicator flag, true if to use KDV H.323.
 *				: bAutoDetect  - Indicator flag for protocol auto adaptive.
 * Output		: N/A.
 *****************************************************************************/
RVAPI RvStatus RVCALLCONV cmSetKeda323(IN HAPP hApp, IN RvBool bKDV323, IN RvBool bAutoDetect);
RVAPI RvStatus RVCALLCONV cmGetCallStats(IN HAPP hApp, OUT int* pnCallConnected, OUT int* pnCallConnecting);

#ifdef _ENABLE_QUANTUM_PROJECT_
RVAPI void RVCALLCONV cmSetQuantumID(IN HAPP hApp, IN RvUint32 dwQuantumID);
RVAPI RvStatus RVCALLCONV cmEnableQuantumCall(IN HAPP hApp, IN RvBool bIsQuantumCall);
#endif
