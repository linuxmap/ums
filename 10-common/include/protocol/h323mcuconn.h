...
enum H323MCUMsgType
{
    CASCADE_MSGTYPE_BEGINNING,
        ...
        H_CASCADE_PARTLIST_RSP,   //[IN]&[OUT]:[TPartListMsg + n*sizeof(TPart)] reponse part list (dwpid ==0 表示对方的MCU)
...
函数名      ：MMcuEncodeToXml
功能        ：业务数据进行XML编码
输入参数说明：[IN]nMsgID         : 级联消息号（H323MCUMsgType）
              [IN]pbyBuffer      : 业务数据内容
              [IN]dwBufLen       : 业务数据内容长度
              [OUT]pbyXmlBuf     : XML编码结果缓存
              [IN]dwXmlBufLen    : XML编码结果缓存长度
返回值说明  ：s32         : XML编码结果长度
====================================================================*/
...
/*====================================================================
函数名      ：MMcuDecodeFromXml
功能        ：XML编码解析为业务数据
输入参数说明：[IN]pXmlBuf           : XML内容缓存
              [IN]nXmlLen           : XML内容长度
              [OUT]pResultBuf       : 输出结果缓存
              [IN]dwResultBufLen    : 输出结果缓存长度
返回值说明  ：s32         : 业务数据长度
====================================================================*/
