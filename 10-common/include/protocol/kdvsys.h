...
#define MAX_WRITE_SECTIONONECE_LEN  (u16)(20*100)//写一次section的最大长度
...
/*====================================================================
    ...
    功能        ：往配置文件中写入一个段内容(一次最大20*100，超过该大小应多次调用该接口写入)，如果不存在则自动创建
    算法实现    ：（可选项）
    引用全局变量：无
    输入参数说明：const s8* lpszProfileName, 文件名（含绝对路径）   
                  const s8* lpszSectionName, Profile中的段名
                  const s8* lpszValue, 新的字串值, NULL则表示删除
				  u16 wCurInSectionOffset,当前写入的段内偏移（如：写单板的时候，配置的单板过多，可能要分多次写）
				  const s8* lpSzSectionKeepKey 段中要保留的关键字名（如：写一整段的时候要保留entrynum的值）
    返回值说明  ：TRUE/FALSE
====================================================================*/
DllExport BOOL32 SetRegKeySection( const s8* lpszProfileName, 
