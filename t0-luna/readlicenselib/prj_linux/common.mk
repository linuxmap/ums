...
ASMS_CLIENT := $(foreach obj,$(OBJS_CLIENT),$(basename $(obj)).s)

ASMS_SERVER := $(foreach obj,$(OBJS_SERVER),$(basename $(obj)).s)
...
OBJS_CLIENT := $(foreach obj,$(OBJS_CLIENT),$(obj).o)

OBJS_SERVER := $(foreach obj,$(OBJS_SERVER),$(obj).o)
...
ifeq ($(LINUX_COMPILER),_HHPPC82xx_)
   LDFLAGS += --static
endif

ifeq ($(LINUX_COMPILER),_HHPPC85xx_)
   LDFLAGS += --static
endif
...
## Rules for making applications----------------(lhq 2004.10.31)

ifneq ($(strip $(APP_TARGET)),)

  all: install cleanobjs_app
...
## Rules for making applications----------------OspClientDemo(lhq 2004.10.31)

ifneq ($(strip $(APP_TARGET_CLIENT)),)

  all: $(APP_TARGET_CLIENT) cleanobjs_client
  
  install: install_inc install_app_client
  
  install_app_client: $(APP_TARGET_CLIENT)
	$(INSTALL) $(APP_TARGET_CLIENT) $(INSTALL_APP_PATH)/$(APP_TARGET_CLIENT)
  
  $(APP_TARGET_CLIENT): $(OBJS_CLIENT)
	$(LD) $(OBJS_CLIENT) -o $(APP_TARGET_CLIENT) $(LDFLAGS)
#	$(OBJDUMP) --syms $(APP_TARGET_CLIENT) | sort | grep " g" > $(APP_TARGET_CLIENT).map

  clean: cleanapp_client
  
  cleanapp_client:
	$(RM) $(APP_TARGET_CLIENT)
...
## Rules for making applications----------------OspServerDemo(lhq 2004.10.31)

ifneq ($(strip $(APP_TARGET_SERVER)),)

  all: $(APP_TARGET_SERVER) cleanobjs_server
  
  install: install_inc install_app_server
  
  install_app_server: $(APP_TARGET_SERVER)
	$(INSTALL) $(APP_TARGET_SERVER) $(INSTALL_APP_PATH)/$(APP_TARGET_SERVER)
  
  $(APP_TARGET_SERVER): $(OBJS_SERVER)
	$(LD) $(OBJS_SERVER) -o $(APP_TARGET_SERVER) $(LDFLAGS)
#	$(OBJDUMP) --syms $(APP_TARGET_SERVER) | sort | grep " g" > $(APP_TARGET_SERVER).map

  clean: cleanapp_server
  
  cleanapp_server:
	$(RM) $(APP_TARGET_SERVER)
	
  cleanobjs_server:
	$(RM) $(OBJS_SERVER) *.pdb *.map
...
cleanobjs:
	$(RM) $(ASMS) $(ASMS_CLIENT) $(ASMS_SERVER)  $(OBJS) $(OBJS_CLIENT) $(OBJS_SERVER) *.pdb *.map
